  ...
  describe("CrossMap") {
    it("finds cross-map relations") {
      whenReady(crossMap.execute(dbSourceId, dbTargetId, Seq("1", "1005101"))) { res =>
        ...
        res should contain(("1005101", "AF468436/#6"))
      }
    }
    ...
    it("finds nothing on either DB ID is negative") {
      whenReady(crossMap.execute(-1, dbTargetId, Seq())) { _ shouldBe Seq() }
      whenReady(crossMap.execute(dbSourceId, -1, Seq())) { _ shouldBe Seq() }
